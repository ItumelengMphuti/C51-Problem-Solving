/*
Week 5 Debugging Challenge — Animal
Adoption System
What is this program supposed to do?
You are working with an animal adoption system.
The program has four main classes/concepts:
● Animal — the parent class that stores an animal's name and provides common
behaviour.
...
class Home {
  ...
  adoptPet(pet) {
    if (!(pet instanceof Animal)) {
      throw new Error(ERRORS.NOT_AN_ANIMAL_ADOPT);
    }
    if (pet instanceof Dog && pet.isAdopted) {
  ...
  removePet(pet) {
    ...
    if (index === -1) {
      throw new Error(ERRORS.NOT_IN_HOME(pet.getName()));
    }
